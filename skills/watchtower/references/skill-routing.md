# Skill Routing for TASKs

When authoring a TASK (`new`), look for an installed skill that fits the work and
name it in the TASK's `Prompt`, so implementer uses it. This list includes skills
installed under `/Users/hiep/.claude/skills` at time of writing. Also scan skills
available in current session and pick any that fit the TASK.
...
| Skill | Use it when a TASK needs |
|-------|--------------------------|
...
For any TASK about UI, website, app screen, redesign, branding, or design images,
pick from these. Default to taste-skill for new frontend work; use narrower skill
only when TASK clearly matches it.

| TASK needs | Use skill |
|------------|-----------|
...
Avoid naming uninstalled historical skills. If TASK needs image-only output and no
...
- While writing a TASK (`new`), explore skills that fit its work - this list plus the
  session's available skills.
- If one fits and changes how implementer should work, name it in that TASK's `Prompt`, for example: "Use /solve to ...".
