...
Maintain repo-local planning files. Keep `next/NEXT.md` as active manifest, `next/CONTEXT.md` as short shared context, TODO briefs under `next/todos/`, and old plans under `next/archive/`.
...
| `archive` | Archive active manifest, context, TODO files, and any legacy verify file | moves active plan files to `next/archive/` | [Archive](#archive) |
...
2. For `new`, `progress`, `archive`, and `verify`, read before writing: `.gitignore`, `next/NEXT.md` if present, `next/CONTEXT.md` if present, `next/NEXT.VERIFY.md` if present, TODO filenames under `next/todos/`, and archive filenames under `next/archive/`. For `implement`, use the minimal load set in Implement. For `next`, skip this broad read; Propose Next owns minimal load.
3. Do not add `/next/` to `.gitignore`. If `next/` is already ignored, keep current state. If tracked, keep tracked.
4. Detect plan shape before routing:
   - New format: `next/NEXT.md` Tracker has `Spec`, `Deps`, and `Context` columns. TODO detail lives under `next/todos/`.
   - Legacy format: `next/NEXT.md` has inline `## TODO N` sections or `next/NEXT.VERIFY.md` exists as active verify file.
...
1. Decide by file presence:
   - `next/NEXT.md` absent -> CREATE.
   - `next/NEXT.md` present with Status `ARCHIVED` or empty Tracker -> CREATE.
   - `next/NEXT.md` present with every Tracker row `DONE` -> ask with `AskUserQuestion`: extend this plan (ADD), or archive it and start fresh (`archive` then CREATE). Do not guess.
   - `next/NEXT.md` present with open TODOs -> ADD.
2. CREATE: write `next/NEXT.md`, `next/CONTEXT.md`, and one file per TODO under `next/todos/`. Do not create `next/NEXT.VERIFY.md` for new-format plans. Assign plan `Slug:` once: `YYYYMMDD-kebab-title`, for example `20260619-arcade-mascot-spotlight`. Archive reuses this slug.
3. ADD: append only requested Tracker rows to `next/NEXT.md` and create matching `next/todos/TODO-NNN-kebab-title.md` files. If added TODOs broaden work past current Title, update Title and shared context only as needed.
4. Open changed files in VS Code (Open In VS Code) after writing them.
5. Legacy plan ADD: add only when existing legacy plan already has clear inline TODO format. If verify mapping is unclear or `next/NEXT.VERIFY.md` is missing, stop and ask before changing legacy files. Do not migrate unless user asks.
...
1. Build one `-g` target per file with absolute paths:
   - `next/NEXT.md` at line 1 on CREATE, else added Tracker row line.
   - `next/CONTEXT.md` at line 1 on CREATE, or on ADD only when context changed.
...
1. Read `next/NEXT.md`. If absent, report no active plan and offer to start one with `new`. Stop.
...
4. New format: read only selected row's `Spec` file. Do not read full TODO files for `DONE` rows. Read `## Handoff` in `next/NEXT.md`.
...
1. New format: read `next/NEXT.md`, then read only TODO files that need verification from Tracker `Spec` column. Run each TODO's `## Verify` checks. Run `## Plan Verify` in `next/NEXT.md` when present.
2. Legacy format: if `next/NEXT.VERIFY.md` exists, use existing checklist exactly as written. Run each checkbox in order and record live results there. If legacy inline plan lacks `NEXT.VERIFY.md`, verify from inline TODO `Expected result` and `## Verification` only when those sections are clear. If not clear, stop and ask.
...
5. Tracker update: for each TODO whose Outcome status is `DONE`, set Tracker Status to `DONE` in `next/NEXT.md`. Leave rows unchanged if any check failed or was skipped. Then recompute plan-level `Status:` header.
...
1. Read `next/NEXT.md` and selected TODO file when new format is active.
...
1. Read latest `next/NEXT.md`. If absent, report no active plan and offer to start one with `new`. Stop.
...
5. New format load set:
   ...
   - Read `next/CONTEXT.md` once when any current work set row's `Context` column requires it.
...
8. Run `## Plan Verify` commands in `next/NEXT.md` when present. Fix failures before claiming done. Report real outcomes.
...
- `next/NEXT.md`: active manifest. Holds plan metadata, Tracker, dependency paths, Handoff, Archive, and optional `## Plan Verify`.
- `next/CONTEXT.md`: short shared context that applies across TODOs. Keep stable and brief.
- `next/todos/TODO-NNN-kebab-title.md`: full TODO spec. Holds `## Brief`, `## Verify`, and `## Outcome`.
- `next/NEXT.VERIFY.md`: legacy verify file only. Do not create for new-format plans.
- `next/archive/<slug>/NEXT.md`: archived manifest.
- `next/archive/<slug>/CONTEXT.md`: archived shared context when present.
- `next/archive/<slug>/todos/`: archived TODO files when present.
- `next/archive/<slug>/NEXT.VERIFY.md`: archived legacy verify file when present.
- Root `NEXT.md`, `NEXT_*.md`, and `NEXT.VERIFY.md` are legacy ignored paths. Do not create new root NEXT files.

## Writing Style

Write every `next/NEXT.md`, `next/CONTEXT.md`, and `next/todos/*.md` in caveman-full style: shortest words, no waste, same logic. Reader still reviews plan files alone, before any code.
...
- File references in plan prose, lists, and tables MUST be markdown links, not plain text or inline code. Use `[path](path)` for repo-relative files, for example `[next/todos/TODO-001-foo.md](next/todos/TODO-001-foo.md)`. Keep raw paths only inside command blocks, file tree blocks, and exact command examples.
...
- `Spec` points to TODO file as markdown link, for example `[next/todos/TODO-NNN-kebab-title.md](next/todos/TODO-NNN-kebab-title.md)`.
- `Deps` points to TODO ids or TODO files whose `## Outcome` must be read before work starts. Use `-` when none.
- `Context` is usually `[next/CONTEXT.md](next/CONTEXT.md)` or `-`.
...
1. Read `Slug:` from `next/NEXT.md`. Build archive directory `next/archive/<slug>/`. If it exists, add `-HHMM` to avoid overwrite.
2. Before moving files, update `next/NEXT.md`: set plan-level `Status: ARCHIVED`, set `Updated:` to current date, and append `- Archived: <YYYY-MM-DD> -> next/archive/<slug>/` under `## Archive`.
3. Move `next/NEXT.md`, `next/CONTEXT.md` if present, and `next/todos/` if present into that archive directory.
4. If `next/NEXT.VERIFY.md` exists, treat it as legacy and archive it in same directory.
...
- `new` does not create `next/NEXT.VERIFY.md` for new-format plans.
...
- `archive` moves `next/NEXT.md`, `next/CONTEXT.md`, `next/todos/`, and any legacy `next/NEXT.VERIFY.md` to `next/archive/<slug>/`.
- Before archiving legacy plan, if `next/NEXT.VERIFY.md` has unchecked `## Pre-commit gate` item, warn user and offer to run it first. If user archives anyway, note open gate in recap.
- `next` mode and "what next?" asks are read-only. Never edit NEXT files when only proposing.
- Do not delete archive files.
- Do not change `.gitignore` to ignore or unignore `next/`.
