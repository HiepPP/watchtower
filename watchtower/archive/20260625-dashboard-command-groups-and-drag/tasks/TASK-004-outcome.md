...
Changed:
- Parser now derives `TASK-NNN` from Spec basename when TODO cell holds title only.
- Parser now falls back to padded Order id when Spec lacks `TASK-NNN`.
- Row title keeps readable Tracker TODO cell text.

Contract:
- Rows still use `todo.id` for implement copy commands.
- Existing `TASK-NNN Title` Tracker cells still parse as before.
...
- Manual VS Code check -> active dashboard row id column shows `TASK-001` through `TASK-004`; title column shows task text.
